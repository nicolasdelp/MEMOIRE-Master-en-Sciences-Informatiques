...
def load_moge_depth_metric(path):
    """
    Loads a 16-bit MoGe PNG and converts it to actual metres.
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog='ClosestPersonFiltering',
        description='Keep only the closest person from a SAM3 mask using MoGe depth.'
    )
    ...
    for folder in sorted(os.listdir(PARENT_FOLDER)):
        ...
        MASKS_FOLDER_PATH = os.path.join(PARENT_FOLDER, folder, "sam3")
        DEPTH_FOLDER_PATH = os.path.join(PARENT_FOLDER, folder, "moge")
        OUTPUT_PATH = os.path.join(PARENT_FOLDER, folder, "sam3_filtered")

        if not (os.path.isdir(IMAGE_FOLDER_PATH)
                and os.path.isdir(MASKS_FOLDER_PATH)
                and os.path.isdir(DEPTH_FOLDER_PATH)):
            print(f"Skip {folder} (missing frames/sam3/moge)")
            continue
