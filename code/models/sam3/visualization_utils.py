...
def save_binary_mask(mask_tensor: torch.Tensor, output_path: str, image_name: str):
    """
    Combine all masks from a 4D tensor into a single binary PNG image.
    """
    # if a pixel is True for any object, it becomes True in the result.
    combined_mask = mask_tensor.detach().any(dim=0).cpu().numpy()

    # The tensor goes from [1, H, W] to [H, W]
    combined_mask = np.squeeze(combined_mask)

    # Convert Bool in uint8 (0 or 255)
    binary_mask = (combined_mask.astype(np.uint8)) * 255

    # Save the binary mask
    os.makedirs(output_path, exist_ok=True)
    base_name = os.path.splitext(os.path.basename(image_name))[0]
    filename = os.path.join(output_path, f"{base_name}.png")
