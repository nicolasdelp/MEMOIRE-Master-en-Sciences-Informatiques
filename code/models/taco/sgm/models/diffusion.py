...
class DiffusionEngine(pl.LightningModule):
    ...
    def init_from_ckpt(self, path: str,) -> None:
        ...
        if self.ablate_unet_scratch:
            ...
            sd = sd2
        in_filters_load = sd['model.diffusion_model.input_blocks.0.0.weight']
        new_state = self.model.state_dict()
        in_filters_current = new_state['diffusion_model.input_blocks.0.0.weight']
        in_shape = in_filters_current.shape
        if in_shape != in_filters_load.shape:
            input_weight = new_state['diffusion_model.input_blocks.0.0.weight']
            print("Manual init: diffusion_model.input_blocks.0.0.weight")
            input_weight.zero_()
            input_weight[:, :8, :, :].copy_(sd['model.diffusion_model.input_blocks.0.0.weight'])
            sd['model.diffusion_model.input_blocks.0.0.weight'] = torch.nn.parameter.Parameter(input_weight)
