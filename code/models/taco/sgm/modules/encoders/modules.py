...
class GeneralConditioner(nn.Module):
    ...
    def __init__(self, emb_models: Union[List, ListConfig]):
        ...
        for n, embconfig in enumerate(emb_models):
            ...
            elif "multi_vae" in embconfig:
                embedder.multi_vae = embconfig["multi_vae"]
            else:
                raise KeyError(
    ...
    def forward(
        ...
        for embedder in self.embedders:
            ...
            with embedding_context():
                ...
                elif hasattr(embedder, "multi_vae"):
                    emb_out = []
                    for k in embedder.multi_vae:
                        emb_out.append(embedder(batch[k]))
            ...
            for emb in emb_out:
                ...
                if embedder.ucg_rate > 0.0 and embedder.legacy_ucg_val is None:
                    # Sprinkle zero multiplication into the embedding;
                    # decisions are made across the current batch dimension (i.e. = per frame!).
        ...
        # before SphericalEmbedder:
        # vector: (28, 768) of float32, which comes from 3x concat (28, 256).
        # after SphericalEmbedder:
        # vector: (28, 896) of float32, comes from concat of:
        #   3x ConcatTimestepEmbedderND (28, 256) + 1x SphericalEmbedder (28, 128).
...
class CameraEmbedder(AbstractEmbModel):
    def __init__(self, embed_dim=128, zero_init=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = x.reshape((*x.shape[:-2], 12))

        h = self.proj(x)
        return h


class SphericalEmbedder(AbstractEmbModel):
    def __init__(self, embed_dim=128, zero_init=False):
        super().__init__()
        self.proj = nn.Linear(13, embed_dim)
        if zero_init:
            self.proj.weight.data.zero_()
            self.proj.bias.data.zero_()

    def forward(self, x):
        # x = (28, 3) tensor of float32.
        assert x.shape[-1] == 3

        # Assume these 3 delta values are given.
        azimuth = x[..., 0]  # radians (so range 2*pi)
        elevation = x[..., 1]  # radians (so range 2*pi)
        radius = x[..., 2]  # meters (so range infinity)

        # Similar to timestep embedding, add a few cos and sin operations for signal amplication.
        azimuth_emb = torch.stack([
            torch.cos(azimuth),
            torch.sin(azimuth),
            torch.cos(azimuth * 2.0),
            torch.sin(azimuth * 2.0),
            torch.cos(azimuth * 4.0),
            torch.sin(azimuth * 4.0),
        ], dim=-1)
        elevation_emb = torch.stack([
            torch.cos(elevation),
            torch.sin(elevation),
            torch.cos(elevation * 2.0),
            torch.sin(elevation * 2.0),
            torch.cos(elevation * 4.0),
            torch.sin(elevation * 4.0),
        ], dim=-1)
        radius_emb = radius[..., None]

        x_emb = torch.cat([azimuth_emb, elevation_emb, radius_emb], dim=-1)
        h = self.proj(x_emb)
        # x_emb = (28, 13) tensor of float32.
        # h = (28, 128) tensor of float32.
        return h
...
class FrozenOpenCLIPImageEmbedder(AbstractEmbModel):
    ...
    def forward(self, image, no_dropout=False):
        # image = (28, 3, 256, 384) of float32.
        z = self.encode_with_vision_transformer(image)
        # z = (28, 1024) tensor of float32.
        tokens = None
...
class VideoPredictionEmbedderWithEncoder(AbstractEmbModel):
    ...
    def forward(
        ...
        # return_val = (28, 4, 32, 48) of float32.
