...
class VACDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            frame_num = len(os.listdir(os.path.join(video_path, 'origin_rgb')))
            ...
            for frame in clip_frames:
                input.append(self.image_transforms(Image.open(os.path.join(video_path, 'origin_rgb', f'{frame:02d}.png')).convert('RGB')))
                # target.append(self.image_transforms(Image.open(os.path.join(video_path, 'amodal_rgb', f'{frame:02d}.png')).convert('RGB')))
                visible_mask.append(self.image_transforms(Image.open(os.path.join(video_path, 'visible_mask', f'{frame:02d}.png')).convert('RGB')))
            fps = 6
            motion_amount = 127
