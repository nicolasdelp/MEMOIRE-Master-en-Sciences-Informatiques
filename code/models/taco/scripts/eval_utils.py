...
def write_video_and_frames(images, dst_dp=None, dst_fp=None, fps=10, save_amodal=False, save_images=True,
    ...
    if save_amodal:
        ...
        for i, image in enumerate(images):
            plt.imsave(os.path.join(dst_dp, f'amodal_rgb_{i:04d}.png'), image[img_height//2:img_height, :, :])

    if save_images:
        os.makedirs(dst_dp, exist_ok=True)
        print(f'[yellow]Saving frames as images to: {dst_dp}')
        for i, image in enumerate(images):
            plt.imsave(os.path.join(dst_dp, f'concat_{i:04d}.png'), image)
