...
class VACDataset(torch.utils.data.Dataset):
    ...
    def __init__(
            ...
            output_frames=14,
            center_crop=True, frame_width=640, frame_height=384,
            input_mode='arbitrary', output_mode='arbitrary',
            motion_bucket_range=[127, 127],
            cond_aug=0.02,
            reverse_prob=0.0, data_gpu=0, folder_name='',
            **kwargs):
        super().__init__()
        self.train = train
    ...
    def __getitem__(self, idx):
        ...
        for frame in clip_frames:
            input.append(self.image_transforms(Image.open(os.path.join(video_path, 'origin_frame', f'origin_frame_{frame:03d}.png')).convert('RGB')))
            visible_mask.append(self.image_transforms(Image.open(os.path.join(video_path, 'visible_mask', f'mask_{frame:03d}.png')).convert('RGB')))
        fps = 6
        motion_amount = 127
...
def test_args():
    ...
    parser.add_argument('--input_example', type=str, default='wild_people')
...
def main(args):
    ...
    data = VACDataset(dset_root=test_config.data.params.dset_root, folder_name=args.input_example, train='val')
        ...
        with torch.no_grad():
            with torch.autocast(**autocast_kwargs):
                ...
                for sample_idx in range(args.num_samples):
                    ...
                    # (Tcm, 4, Hl, Wl) = (14, 4, 32, 48) array of float32.
                    video_dict['vertcat'] = \
                    torch.cat([video_dict['cond_video'], 
                               video_dict['sampled_video']], dim=2)
                    eval_utils.write_video_and_frames(video_dict['vertcat'].permute(0, 2, 3, 1).detach().cpu().numpy(), 
                                                      dst_dp=os.path.join(args.output, f'{args.input_example}_{cur_num:04d}'), save_amodal=True, save_images=True, save_mp4=False)
                    cur_num += 1
