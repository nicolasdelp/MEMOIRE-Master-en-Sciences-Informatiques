'''
Adapted from the GCD repository
'''
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (
            ...
            if 'video_dict' in images:
                ...
                if 'extra' in video_dict:
                    scene_idx = video_dict['extra']['video_id'][0].replace('/', '_')
                    fps = video_dict['extra']['fps_id'][0].item()
                    motion_bucket = video_dict['extra']['motion_bucket_id'][0].item()

                    if 'scaled_relative_angles' in video_dict['extra']:
                        sra = video_dict['extra']['scaled_relative_angles'][-1].numpy()
                        azimuth = sra[0] * 180.0 / np.pi
                        elevation = sra[1]
                        if video_dict['extra']['dset'][0].item() != 2:
                            # Kubric => angle (radians); ParallelDomain => distance (meters).
                ...
                for (k, v) in video_dict.items():
                    if torch.is_tensor(v):
                        ...
                        # New format that adds useful information:
                        filename = (f'{k}_ep{pl_module.current_epoch:03d}'
                                    f'_gs{pl_module.global_step:06d}'
                                    f'_si{scene_idx}_fr{fps}_mb{motion_bucket}.mp4')
                        path = os.path.join(root, filename)
                        cv = convert_video(v)
                        ...
                        if fps >= 2:
                            ...
                            used_fps = 6
                        my_save_video(path, cv, fps=used_fps, quality=9)

            # NOTE: This part handles inputs, reconstructions, fps_id, motion_bucket_id, etc.
            for k in images:
...
def init_wandb(save_dir, opt, config, group_name, name_str):
    ...
    if opt.debug:
        print("Running in debug mode, initializing wandb in offline mode to avoid syncing with the cloud.")
...
def my_save_video(dst_fp, frames, fps, quality):
...
if __name__ == "__main__":
    ...
    try:
        ...
        if ckpt_resume_path:
            config.model.params.ckpt_path = ckpt_resume_path
        ...
        default_logger_cfgs = {
            "wandb": {
                ...
                "params": {
                    "name": nowname,
                    "entity": "nicolasdelp-universit-de-mons",
                    "save_dir": logdir,
                    "offline": True,
                    "id": nowname,
                    "project": opt.projectname,
        ...
        # signal.signal(signal.SIGUSR2, divein)  # not necessary with VSCode debugger
